...
const logger = log4js.getLogger('pev-network');
...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name ) => {
    try {
        ...
        if (fcn === "castElectionVote") {
            let timestamp = new Date (); 
            result = await contract.submitTransaction(fcn, args[0], args[1], timestamp);
            message = `Successfully vote casted in election with key ${args[0]}`
            // console.log("TxID:", result.GetTxID());
         }
         else if (fcn === "castPollVote") {
            let timestamp = new Date (); 
            result = await contract.submitTransaction(fcn, args[0], args[1], args[2], timestamp);
            message = `Successfully vote casted in poll ${args[0]}`
            // console.log("TxID:", result.GetTxID());
         }
